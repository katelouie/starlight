...
class ChartWheel:
    ...
    def __init__(self, size: int = 600, style: Optional[Dict[str, Any]] = None):
        ...
        self.radius_planets = size * 0.30  # Planet placement (increased for info rings)
        self.radius_inner = size * 0.16  # Inner clear area (reduced to fit retrograde markers better)
        ...
        self.style = {
            ...
            "show_aspects": True,
            # Planet information styling
            "show_planet_info": True,
            "planet_info_offset": 18,  # Distance from planet symbol
            "planet_info_size": "10px",
            "planet_info_color": "#2C3E50",
            "planet_info_font": "Arial, sans-serif",
        }
    ...
    def polar_to_cartesian(
        self, angle_degrees: float, radius: float
    ) -> Tuple[float, float]:
        """
    ...
    def _draw_degree_ticks(self, dwg: svgwrite.Drawing) -> None:
        ...
        for degree in range(0, 360, 5):
            ...
            x_inner, y_inner = self.polar_to_cartesian(
                degree, self.radius_outer - tick_length
            )
    ...
    def draw_house_divisions(self, dwg: svgwrite.Drawing, chart: Chart) -> None:
        ...
        for i, cusp_degree in enumerate(chart.cusps):
            ...
            # Add house number at the actual middle of the house segment
            house_number = i + 1
            next_cusp = chart.cusps[(i + 1) % 12]
            
            # Calculate the actual middle of this house segment
            if next_cusp < cusp_degree:  # Handle wrapping around 360°
                house_middle = cusp_degree + ((next_cusp + 360 - cusp_degree) / 2)
            else:
                house_middle = cusp_degree + ((next_cusp - cusp_degree) / 2)
            house_middle = house_middle % 360
    ...
    def _draw_house_sector(
        ...
        # Draw the sector
        dwg.add(dwg.path(d=path_data, fill=fill_color, stroke="none", opacity=0.3))
    ...
    def draw_zodiac_ring(self, dwg: svgwrite.Drawing, chart: Chart) -> None:
        ...
        for i in range(12):
            ...
            # Draw sign division line - solid from zodiac to outer edge
            x_outer, y_outer = self.polar_to_cartesian(actual_degree, self.radius_outer)
            x_zodiac, y_zodiac = self.polar_to_cartesian(
                actual_degree, self.radius_zodiac
            )
            x_inner, y_inner = self.polar_to_cartesian(actual_degree, self.radius_inner)

            # Solid line from zodiac ring to outer edge
            dwg.add(
                dwg.line(
            ...
            # Dashed line from inner circle to zodiac ring
            dwg.add(
                dwg.line(
                    start=(x_inner, y_inner),
                    end=(x_zodiac, y_zodiac),
                    stroke=self.style["zodiac_line_color"],
                    stroke_width=self.style["zodiac_line_width"] * 0.7,  # Slightly thinner
                    stroke_dasharray="2,3",  # Dashed pattern
    ...
    def draw_planets(self, dwg: svgwrite.Drawing, chart: Chart) -> None:
        ...
        planet_positions = self._calculate_planet_positions_with_collision_detection(
            chart
        )
        ...
        for planet, position in planet_positions.items():
            ...
            planet_color = (
                self.style["planet_retro_color"]
                if planet.is_retro
                else self.style["planet_color"]
            )
            ...
            if position["adjusted"]:
                x_actual, y_actual = self.polar_to_cartesian(
                    planet.long, self.radius_planets
                )
                dwg.add(
                    dwg.line(
            ...
            # Add planet information (sign symbol and degrees/minutes)
            if self.style.get("show_planet_info", True):
                self._draw_planet_info(dwg, planet, x_planet, y_planet, position["adjusted"])

    def _draw_planet_info(self, dwg: svgwrite.Drawing, planet, x_planet: float, y_planet: float, is_adjusted: bool = False) -> None:
        """
        Draw planet information (sign, degrees, minutes, Rx) in concentric rings inward from planet.
        Each piece of information is displayed on progressively smaller radii along the same polar vector.
        
        Args:
            dwg: SVG Drawing object
            planet: Planet object with sign and position data
            x_planet: X coordinate of planet symbol
            y_planet: Y coordinate of planet symbol
            is_adjusted: Whether the planet position was adjusted for collision detection
        """
        # Get sign symbol from DIGNITIES
        sign_symbol = DIGNITIES.get(planet.sign, {}).get("symbol", planet.sign[:3])
        
        # Format degrees and minutes separately for individual placement
        degrees = int(planet.sign_deg)
        minutes = int(planet.sign_min)
        degree_text = f"{degrees:02d}°"
        minute_text = f"{minutes:02d}'"
        
        # Use the original astronomical position for planet info placement
        planet_angle = planet.long
        
        # Define ring radii - progressively smaller inward from original planet position
        sign_radius = self.radius_planets - 22     # Sign symbol ring
        degree_radius = self.radius_planets - 38   # Degree ring  
        minute_radius = self.radius_planets - 50   # Minute ring
        retro_radius = self.radius_planets - 62    # Retrograde indicator ring
        
        # Calculate positions for each ring on the same polar ray as original position
        sign_x, sign_y = self.polar_to_cartesian(planet_angle, sign_radius)
        degree_x, degree_y = self.polar_to_cartesian(planet_angle, degree_radius)
        minute_x, minute_y = self.polar_to_cartesian(planet_angle, minute_radius)
        ...
        dwg.add(
            dwg.text(
                ...
                text_anchor="middle",
                dominant_baseline="middle",
                font_size="11px",  # Slightly larger for sign
                fill=self.style["planet_info_color"],
                font_family=self.style["zodiac_font_family"],  # Use zodiac font for symbols
                font_weight="normal",
            )
        )
        ...
        dwg.add(
            dwg.text(
                ...
                font_size="9px",
                fill=self.style["planet_info_color"],
                font_family=self.style["planet_info_font"],
                font_weight="normal",
            )
        )
        ...
        dwg.add(
            dwg.text(
                minute_text,
                insert=(minute_x, minute_y),
                text_anchor="middle",
                dominant_baseline="middle",
                font_size="8px",  # Smallest for minutes
                fill=self.style["planet_info_color"],
                font_family=self.style["planet_info_font"],
                font_weight="normal",
            )
        ...
        # Draw retrograde indicator if present
        if hasattr(planet, 'is_retro') and planet.is_retro:
            retro_x, retro_y = self.polar_to_cartesian(planet_angle, retro_radius)
            dwg.add(
                dwg.text(
                    "℞",
                    insert=(retro_x, retro_y),
                    text_anchor="middle",
                    dominant_baseline="middle",
                    font_size="8px",
                    fill="#E74C3C",  # Use consistent red color from style
                    font_family=self.style["planet_info_font"],
                    font_weight="bold",
                )
            )

    def _calculate_planet_positions_with_collision_detection(
        self, chart: Chart
    ) -> Dict:
        """
        ...
        for planet in sorted_planets:
            ...
            x_ideal, y_ideal = self.polar_to_cartesian(planet.long, self.radius_planets)

            # Check for collisions with already placed planets
            collision_detected = False
            for placed_pos in placed_positions:
                distance = math.sqrt(
                    (x_ideal - placed_pos["x"]) ** 2 + (y_ideal - placed_pos["y"]) ** 2
                )
    ...
    def _find_alternative_position(
        self, original_longitude: float, placed_positions: list, min_distance: float
    ) -> Dict:
        """
    ...
    def draw_angles(self, dwg: svgwrite.Drawing, chart: Chart) -> None:
        ...
        for angle in chart.angles:
            if angle.name in main_angles:
                ...
                x_inner, y_inner = self.polar_to_cartesian(
                    angle.long, self.radius_inner
                )
                x_outer, y_outer = self.polar_to_cartesian(
                    angle.long, self.radius_outer
                )
            ...
            for planet2 in valid_planets[i + 1 :]:  # Avoid duplicate pairs
                ...
                for aspect_name, aspect_data in ASPECTS.items():
                    is_aspect, orb, distance, movement = planet1.aspect(
                        planet2, aspect_data["degree"], aspect_data["orb"]
                    )

                    if is_aspect:
                        self._draw_aspect_line(dwg, planet1, planet2, aspect_name, orb)
                        break  # Only draw one aspect per planet pair

    def _draw_aspect_line(
        self, dwg: svgwrite.Drawing, planet1, planet2, aspect_name: str, orb: float
    ) -> None:
        """
    ...
    def _create_moon_phase_symbol(
        ...
        # Create the moon phase visualization using accurate path-based method
    ...
    def _draw_moon_phase_accurate(
        ...
        """
        Draw accurate moon phase using path-based approach with curved terminators.

        This creates the correct curved terminator line that accurately represents
        how the moon appears from Earth using elliptical terminator paths.
...
def draw_moon_phase_standalone(
    ...
    """
    Create a standalone moon phase visualization SVG using accurate path-based rendering.
    ...
    # Set phase angle based on waxing status and phase fraction
    # More accurate phase angle calculation
    if waxing:
        # Waxing: 0° (new) to 180° (full)
        phase_angle = illuminated_fraction * 180
    else:
        # Waning: 180° (full) to 360° (new)
        phase_angle = 180 + (1 - illuminated_fraction) * 180

    mock_moon = MockMoon(illuminated_fraction, phase_angle)

    # Create moon phase symbol using the accurate renderer
