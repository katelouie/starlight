...
- **Essential & accidental dignities** for both traditional and modern rulerships
...
**Performance** - Advanced caching system makes repeated calculations blazing fast
...
native = Native(
    datetime_input=datetime(1994, 1, 6, 11, 47),  # Naive or timezone-aware
    location_input="San Francisco, CA"             # City name or (lat, lon)
)
...
print(f"Sun: {sun.longitude:.2f}° in {sun.sign}, House {sun.house}")

moon = chart.get_object("Moon")
print(f"Moon: {moon.longitude:.2f}° in {moon.sign}, House {moon.house}")
print(f"Moon Phase: {moon.phase_data.phase_name} ({moon.phase_data.illumination:.1%} illuminated)")
```
...
for planet in chart.get_planets():
    ...
    print(f"{planet.name:8} {planet.longitude:6.2f}° {planet.sign:12} House {planet.house} {retro}")

# Get aspects
for aspect in chart.aspects:
    print(f"{aspect.object1.name:8} {aspect.aspect_name:12} {aspect.object2.name:8} "
          f"(orb: {aspect.orb:.2f}°)")

# Get house cusps
houses = chart.get_house_cusps()  # Returns HouseCusps for default system
for i, cusp_long in enumerate(houses.cusps, start=1):
    print(f"House {i:2}: {cusp_long:.2f}°")
```
...
print(f"1st House Cusp: {whole_sign_cusps.cusps[0]:.2f}°")
```
...
report = (ReportBuilder()
    ...
    .render(format="rich_table"))    # Rich terminal formatting
...
(ReportBuilder()
    ...
    .render(file="my_chart.txt", format="plain_table"))
```
...
Starlight includes a production-ready CLI for quick chart generation:
...
- **Configurable Orbs**: Simple, Luminaries-specific, or Complex (aspect-specific) orb engines
...
- **[ARCHITECTURE_QUICK_REFERENCE.md](docs/development/ARCHITECTURE_QUICK_REFERENCE.md)** - System design overview
...
| Feature | Starlight | Kerykeion | Immanuel | Flatlib |
|---------|-----------|-----------|----------|---------|
| **Architecture** | Protocol-driven, composable | Class-based | Class-based | Function-based |
| **Type Hints** | ✅ Full | ❌ None | ✅ Partial | ❌ None |
| **Builder Pattern** | ✅ Fluent API | ❌ | ❌ | ❌ |
| **Multiple House Systems** | ✅ Simultaneous | ✅ Sequential | ✅ Sequential | ✅ Sequential |
| **Caching System** | ✅ Advanced | ❌ | ❌ | ❌ |
| **SVG Charts** | ✅ Layered system | ✅ Basic | ❌ | ❌ |
| **Terminal Reports** | ✅ Rich formatting | ❌ | ❌ | ❌ |
| **Extensibility** | ✅ Protocols | ⚠️ Inheritance | ⚠️ Inheritance | ⚠️ Functions |
| **Arabic Parts** | ✅ 25+ (sect-aware) | ❌ | ✅ Basic | ✅ Traditional |
| **Dignities** | ✅ Comprehensive | ❌ | ✅ Basic | ✅ Traditional |
| **Python Version** | 3.11+ | 3.9+ | 3.10+ | 3.x |

**Choose Starlight if you want:**
- Clean, maintainable code with full type safety
- Extensible architecture for custom calculations
- Multiple house systems in a single chart
- Fast performance with automatic caching
- Beautiful terminal output for debugging
- Modern Python development experience

**Choose other libraries if you need:**
- **Kerykeion**: AI/LLM integration, simpler API for basic charts
- **Immanuel**: Astro.com compatibility, mature ecosystem
- **Flatlib**: Specialized traditional astrology techniques
